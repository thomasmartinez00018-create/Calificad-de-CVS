...
const Dashboard: React.FC<DashboardProps> = ({ candidates, onUpdateStatus, onDelete, onClearAll }) => {
  ...
  const getCardTheme = (c: Candidate) => {
    if (c.status === CandidateStatus.DESCARTADO) return {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-6 duration-1000 max-w-5xl mx-auto pb-32">
      <header className="flex justify-between items-center px-2">
        <div>
          <h2 className="text-4xl font-black text-slate-900 tracking-tighter italic uppercase underline decoration-slate-900 decoration-2 underline-offset-8">Panel de Selección</h2>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.3em] mt-4">V3.5 Sincronización de Color</p>
        </div>
        {candidates.length > 0 && (
          <button 
            onClick={(e) => {
            ...
            className="group flex items-center gap-2 bg-white border border-slate-200 px-5 py-3 rounded-2xl shadow-sm hover:border-red-500 hover:bg-red-50 transition-all active:scale-95"
          >
      ...
      <div className="relative group">
        ...
        <input
          type="text"
          placeholder="Buscar por nombre, zona o habilidades clave..."
          className="w-full bg-white pl-14 pr-8 py-6 rounded-[2rem] border-none shadow-xl shadow-slate-200/50 text-sm font-bold focus:ring-4 focus:ring-slate-900/10 transition-all outline-none placeholder:text-slate-300"
          value={search}
        ...
        {filtered.map(c => {
          ...
          return (
            <div 
              ...
              className={`${theme.bg} ${theme.opacity} rounded-[2.5rem] border-2 ${selectedId === c.id ? 'border-slate-900 shadow-2xl scale-[1.01]' : `${theme.border} shadow-sm hover:shadow-lg`} transition-all overflow-hidden cursor-pointer`}
            >
              <div className="p-8 flex items-center gap-8">
                {/* Badge de Score con Color Dinámico */}
                <div className={`w-16 h-16 rounded-2xl ${theme.accent} flex flex-col items-center justify-center shrink-0 shadow-lg`}>
                  ...
                  <span className="text-[8px] font-bold text-white/60 uppercase mt-0.5 tracking-tighter">PTS</span>
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3">
                    <h3 className="text-2xl font-black text-slate-900 truncate tracking-tighter uppercase italic">{c.nombre}</h3>
                    <span className={`text-[8px] font-black px-2 py-1 rounded-md bg-slate-100 ${theme.text} uppercase tracking-widest`}>
                      {theme.label}
                    </span>
                  </div>
              ...
              {selectedId === c.id && (
                <div className={`px-8 pb-8 pt-4 ${c.status === CandidateStatus.DESCARTADO ? 'bg-red-50/50' : 'bg-slate-50'} space-y-6 animate-in slide-in-from-top-4 duration-300`}>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center">
                      ...
                      <p className="text-xs font-bold text-slate-700">{c.telefono || 'Sin dato'}</p>
                    </div>
                    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center">
                    ...
                    <div className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-center">
                      <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">Zona</p>
                    ...
                    <div className={`${theme.accent} p-5 rounded-2xl shadow-xl flex flex-col justify-center border-l-4 border-white/20`}>
                      <p className="text-[9px] font-black text-white/50 uppercase tracking-widest mb-1 italic">Calidad IA</p>
                      <p className="text-sm font-black text-white italic">{c.ai_quality_score}% Match</p>
                    </div>
                  </div>

                  <div className="bg-white p-6 rounded-[2rem] border border-slate-100 space-y-4">
                     <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Resumen de Competencias</h4>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-3">
                      ...
                      <div className="bg-white p-7 rounded-[2.5rem] border border-slate-100 text-sm font-bold text-slate-600 leading-relaxed italic">
                        "{c.resumen}"
                      </div>
                    </div>
                    <div className="space-y-3">
                      <h4 className="text-[10px] font-black text-emerald-600 uppercase tracking-widest px-1 text-right italic">Guía Estratégica de Entrevista</h4>
                      <div className="space-y-2">
                        {c.preguntasEntrevista.map((q, i) => (
                          <div key={i} className="flex gap-4 bg-white p-4 rounded-2xl border border-slate-100 items-center hover:border-slate-900 transition-all">
                  ...
                  <div className="flex flex-col md:flex-row justify-between items-center pt-8 border-t border-slate-200 gap-4">
                    ...
                    <div className="flex gap-3 w-full md:w-auto">
                      <button 
                        ...
                        className={`flex-1 md:flex-none px-10 py-5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${c.status === CandidateStatus.DESCARTADO ? 'bg-red-600 text-white shadow-lg shadow-red-200' : 'bg-white border border-slate-200 text-slate-400 hover:border-red-500 hover:text-red-500'}`}
                      >
                        Descartar
                      </button>
                      <button 
                        onClick={(e) => { e.stopPropagation(); onUpdateStatus(c.id, CandidateStatus.ENTREVISTA); }} 
                        className={`flex-1 md:flex-none px-12 py-5 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${c.status === CandidateStatus.ENTREVISTA ? 'bg-emerald-600 text-white shadow-xl shadow-emerald-200' : 'bg-slate-900 text-white hover:scale-[1.03] active:scale-95'}`}
                      >
                        {c.status === CandidateStatus.ENTREVISTA ? 'Cita Agendada' : 'Llamar / Citar'}
                      </button>
                    </div>
                  </div>
                </div>
